...
export function ContractViewer({ partner, planDescription, onClose }: ContractViewerProps) {
  ...
  const getContractText = () => {
    const clause2 = partner.payment_type === 'misto'
      ? `O(A) CONTRATADO(A) opta pelo plano "${partner.plan_name}" na modalidade MISTA (Financeiro e Permuta), com valor mensal de ${formatCurrency(partner.monthly_price)} (${valorExtenso(partner.monthly_price)}), podendo a contraprestação ser efetuada em dinheiro (espécie/PIX) e/ou em permuta comercial de produtos/serviços, de comum acordo entre as partes.${planDescription ? `\nEspecificações do plano: ${planDescription}` : ''}`
    ...
    const clause6 = partner.payment_type === 'misto'
      ? `A contraprestação mensal no valor de ${formatCurrency(partner.monthly_price)} (${valorExtenso(partner.monthly_price)}) será realizada mediante pagamento em dinheiro (espécie/PIX), boleto ou link de pagamento disponibilizado pela CONTRATANTE, e/ou através da entrega de produtos/serviços em permuta na sede da CONTRATANTE, conforme alinhamento prévio entre as partes.`
      : partner.payment_type === 'permuta'
